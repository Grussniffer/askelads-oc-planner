...
Tampermonkey/Torn PDA userscript for showing each faction member their personal OC Planner recommendation inside Torn.
...
On Torn PDA, the script falls back to PDA's `PDA_httpGet`/`PDA_httpPost` helpers and browser `localStorage` when Tampermonkey `GM_*` APIs are not available.
